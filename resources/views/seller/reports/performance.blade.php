...
<div class="mb-8 flex items-center justify-between">
    <div>
        <h1 class="text-3xl font-bold text-gray-900">Laporan Daftar Produk Berdasarkan Rating</h1>
        <p class="text-gray-600 mt-1">SRS-MartPlace-13 - Diurutkan berdasarkan rating tertinggi</p>
    </div>
    <a href="{{ route('seller.reports.performance.pdf') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    <div class="bg-gradient-to-r from-yellow-400 to-orange-500 rounded-lg p-6 text-white">
        <p class="text-yellow-100 text-sm font-medium">Produk Bintang 5</p>
        <p class="text-4xl font-bold mt-2">{{ $products->where('avg_rating', '>=', 4.5)->count() }}</p>
    </div>
    <div class="bg-gradient-to-r from-green-500 to-teal-600 rounded-lg p-6 text-white">
        ...
        <p class="text-4xl font-bold mt-2">{{ $products->avg('avg_rating') > 0 ? number_format($products->avg('avg_rating'), 2) : '0.00' }}</p>
    </div>
    <div class="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-lg p-6 text-white">
        <p class="text-purple-100 text-sm font-medium">Total Review</p>
        <p class="text-4xl font-bold mt-2">{{ number_format($products->sum('total_reviews')) }}</p>
    </div>
</div>
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4 text-sm text-gray-900">
                        <div class="flex items-center gap-2">
                            {{ $index + 1 }}
                            @if($index < 3 && $product->avg_rating >= 4.5)
                            <span class="text-lg">
                                @if($index == 0)🥇
                                @elseif($index == 1)🥈
                                @else🥉
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <span class="inline-block mt-1 px-2 py-0.5 bg-yellow-100 text-yellow-700 text-xs font-medium rounded">★ Top Rated</span>
                                @endif
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4 text-center">
                        <div class="flex flex-col items-center">
                            ...
                            <span class="text-sm font-bold text-gray-900 mt-1">{{ number_format($product->avg_rating, 2) }}</span>
                            <span class="text-xs text-gray-500">({{ $product->total_reviews }} review)</span>
                        </div>
                    </td>
                    <td class="px-6 py-4 text-center">
