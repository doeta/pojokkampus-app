...
<div class="mb-6 flex items-center justify-between">
    <div>
        <h1 class="text-2xl font-bold text-gray-800">Laporan Daftar Produk Berdasarkan Stock</h1>
        <p class="text-sm text-gray-500 mt-1">Daftar produk diurutkan dari stok terbanyak</p>
    </div>
    <a href="{{ route('seller.reports.stock.pdf') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-gray-800 text-white text-sm font-medium rounded hover:bg-gray-700 transition-colors">
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
    ...
    <div class="bg-white rounded border border-gray-200 p-4">
        <p class="text-gray-500 text-xs font-medium uppercase tracking-wider">Total Stok Tersedia</p>
        <p class="text-2xl font-bold text-gray-800 mt-1">{{ number_format($products->sum('stock')) }}</p>
    </div>
</div>
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-center text-gray-600">{{ number_format($product->avg_rating, 2) }}</td>
                    <td class="px-4 py-3 text-center font-medium {{ $product->stock < 5 ? 'text-red-600' : 'text-gray-900' }}">
                        {{ $product->stock }}
                    </td>
                </tr>
                @endforeach
